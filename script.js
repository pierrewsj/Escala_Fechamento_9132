...
async function aplicarSituacaoEscala() {
  ...
  try {
    ...
    for (let dia = diaInicio; dia <= diaFim; dia += 1) { const atual = decomporDiaEscala(pessoa.dias[dia - 1]); pessoa.dias[dia - 1] = comporDiaEscala(atual.posto, codigo); }
    salvarCacheMes();
    atualizarInterfaceCompleta({ preservarAdmin: true });
...
async function removerSituacaoEscala() {
  ...
  try {
    ...
    for (let dia = diaInicio; dia <= diaFim; dia += 1) {
      ...
      if (atual.situacao) {
        pessoa.dias[dia - 1] = comporDiaEscala(atual.posto, '');
        removidos += 1;
      }
    }
...
async function iniciar() {
  if ($('versaoApp')) $('versaoApp').textContent = `Versão ${APP.versao || '2.4.4'}`;
