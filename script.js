...
const STORAGE = Object.freeze({
  mes: 'escala9132MesSelecionadoV2',
...
function mesDaDataISO(dataISO) {
  return /^\d{4}-\d{2}-\d{2}$/.test(dataISO || '') ? dataISO.slice(0, 7) : mesSelecionado;
}

function diasNoMes(mesAno = mesSelecionado) {
  const [ano, mes] = String(mesAno).split('-').map(Number);
  if (!ano || !mes) return 31;
  return new Date(ano, mes, 0).getDate();
}

function diasDisponiveis(mesAno = mesSelecionado) {
  return Array.from({ length: diasNoMes(mesAno) }, (_, indice) => indice + 1);
}
...
function hojePertenceAoMesSelecionado() {
  return mesSelecionado === mesAtualISO();
}

function diaParaDestaque() {
  return hojePertenceAoMesSelecionado() ? new Date().getDate() : -1;
}
...
function normalizarDadosAtuais() {
  ...
  ESCALA = (Array.isArray(ESCALA) ? ESCALA : []).map((pessoa) => ({
    ...
    turno: pessoa.turno || CONFIG.turno || '1º Turno',
...
function pessoasPorTurno(turno = 'todos') {
  ...
  return turno === 'todos' ? pessoas : pessoas.filter((pessoa) => pessoa.turno === turno);
}
...
/* =========================
   Carregamento e sincronização do mês
   ========================= */
async function carregarMes(mesAno, { forcar = false, silencioso = false } = {}) {
  if (carregandoMes) return;
  const mesValido = /^\d{4}-\d{2}$/.test(mesAno || '') ? mesAno : mesAtualISO();
  mesSelecionado = mesValido;
  localStorage.setItem(STORAGE.mes, mesSelecionado);
  if ($('mesConsulta')) $('mesConsulta').value = mesSelecionado;
  if ($('adminMesAtual')) $('adminMesAtual').textContent = formatarMes(mesSelecionado);
  carregandoMes = true;

  const cache = carregarCacheMes(mesSelecionado);
  if (cache && !forcar) {
    aplicarDados(cache, 'cache local');
    atualizarInterfaceCompleta();
  }

  try {
    if (!navigator.onLine) throw new Error('Sem conexão.');
    const resposta = await apiGet('bootstrap', { mesAno: mesSelecionado, equipe: APP.equipe || CONFIG.equipe });
    aplicarDados({ postos: resposta.postos, escala: resposta.escala }, 'Google Sheets');
    salvarCacheMes(mesSelecionado, {
    ...
    if (!cache) {
      ...
      aplicarDados(padrao, 'dados iniciais');
      salvarCacheMes(mesSelecionado);
    }
...
async function recarregarMesAtual(mostrarAviso = false) {
  await carregarMes(mesSelecionado, { forcar: true, silencioso: !mostrarAviso });
}

async function trocarMesConsulta() {
  const novoMes = $('mesConsulta')?.value || mesAtualISO();
  await carregarMes(novoMes, { forcar: true, silencioso: true });
  mostrarHoje(false);
}

function atualizarInformacoesMes() {
  if ($('mesExibidoInfo')) $('mesExibidoInfo').textContent = `Escala: ${formatarMes(mesSelecionado)}`;
  if ($('sincronizacaoInfo')) $('sincronizacaoInfo').textContent = origemDadosAtual;
  if ($('adminMesAtual')) $('adminMesAtual').textContent = formatarMes(mesSelecionado);
}
...
function preencherSelectTurnos() {
  ...
  if ($('selectTurnoConsulta')) {
    $('selectTurnoConsulta').innerHTML = valores;
    $('selectTurnoConsulta').value = turnosDisponiveis().includes(atual) || atual === 'todos' ? atual : CONFIG.turno;
  }
  const turnoRelatorioAtual = $('relatorioTurno')?.value || 'todos';
  if ($('relatorioTurno')) {
    $('relatorioTurno').innerHTML = valores;
    $('relatorioTurno').value = turnoRelatorioAtual;
  }
}
...
function buscarPorNome(id) {
  ...
  $('resultadoConteudo').innerHTML = `
    <div class="lista-cards">
      <div class="card"><h2>${escaparHTML(pessoa.nome)}</h2><p class="muted">${escaparHTML(pessoa.turno)} • ${escaparHTML(formatarMes(mesSelecionado))}</p></div>
...
function buscarPorDia(dia) {
  ...
  $('resultadoConteudo').innerHTML = `
    <div class="lista-cards">
      <div class="card"><h2>Escala do dia ${String(dia).padStart(2, '0')}</h2><p class="muted">${escaparHTML(formatarMes(mesSelecionado))} • ${escaparHTML(tituloTurno)}</p></div>
...
function buscarPorPosto(postoBusca) {
  ...
  $('resultadoConteudo').innerHTML = `
    <div class="lista-cards">
      <div class="card"><h2>Local / posto ${escaparHTML(postoBusca)}</h2><p class="muted">${escaparHTML(postoDescricao(postoBusca))} • ${escaparHTML(formatarMes(mesSelecionado))}</p></div>
      ${itens.length ? itens.join('') : '<div class="resultado-vazio">Nenhum resultado encontrado.</div>'}
    </div>`;
}

async function mostrarHoje(forcarMesAtual = false) {
  if (forcarMesAtual && mesSelecionado !== mesAtualISO()) await carregarMes(mesAtualISO(), { forcar: true, silencioso: true });
  const dia = hojePertenceAoMesSelecionado() ? Math.min(new Date().getDate(), diasNoMes()) : 1;
...
function restaurarRascunhoCarga() {
  try {
    ...
    if ($('cargaColaborador') && colaboradorPorId(rascunho.colaboradorId)) $('cargaColaborador').value = rascunho.colaboradorId;
...
function preencherSelectColaboradoresCargas() {
  ...
  campo.innerHTML = '<option value="">Selecione o colaborador</option>' + pessoasAtivas()
    .slice().sort((a, b) => `${a.turno}${a.nome}`.localeCompare(`${b.turno}${b.nome}`, 'pt-BR'))
    .map((pessoa) => `<option value="${escaparHTML(pessoa.id)}">${escaparHTML(pessoa.nome)} • ${escaparHTML(pessoa.turno)}</option>`)
    .join('');
  if (colaboradorPorId(valorAtual)) campo.value = valorAtual;
}
...
async function preencherDadosCargaPorColaborador(resetarSetorManual = false, ajustarMesPelaData = true) {
  const data = $('cargaData')?.value || dataISOHoje();
  const mesData = mesDaDataISO(data);
  if (ajustarMesPelaData && mesData !== mesSelecionado && !carregandoMes) {
    await carregarMes(mesData, { forcar: true, silencioso: true });
    preencherSelectColaboradoresCargas();
  }

  const pessoa = colaboradorPorId($('cargaColaborador')?.value || '');
  if (resetarSetorManual && $('cargaSetor')) {
  ...
  const posto = pessoa.dias[Math.max(0, Math.min(30, diaDaDataCarga() - 1))] || '';
  const setorAuto = posto ? `${posto} - ${postoDescricao(posto)}` : 'Sem posto definido';
  $('cargaTurno').value = pessoa.turno;
  $('cargaTurnoInfo').textContent = pessoa.turno;
  $('cargaSetor').dataset.auto = setorAuto;
  if ($('cargaSetor').dataset.manual !== 'true') {
    $('cargaSetor').value = setorAuto;
    $('cargaSetorInfo').textContent = setorAuto;
  }
...
function prepararModuloCargas() {
  ...
  preencherDadosCargaPorColaborador(false, false);
...
function prepararRelatorios() {
  ...
  const inicioMes = `${mesDaDataISO(hoje)}-01`;
  if ($('relatorioDataInicio') && !$('relatorioDataInicio').value) $('relatorioDataInicio').value = inicioMes;
  if ($('relatorioDataFim') && !$('relatorioDataFim').value) $('relatorioDataFim').value = hoje;
  preencherSelectTurnos();
}

function normalizarRegistroRelatorio(item) {
  const dataISO = item.dataISO || converterDataBRparaISO(item.data || '');
  return {
    dataISO,
    data: item.data || formatarDataBR(dataISO),
    colaborador: item.colaborador || '',
    colaboradorId: item.colaboradorId || '',
    registro: item.registro || '',
    turno: item.turno || '',
    setorPosto: item.setorPosto || item.setor || '',
...
function filtrarHistoricoLocal(inicio, fim, turno) {
  return historicoCargasLocal().map(normalizarRegistroRelatorio).filter((item) => {
    const dentroPeriodo = item.dataISO && item.dataISO >= inicio && item.dataISO <= fim;
    return dentroPeriodo && (turno === 'todos' || item.turno === turno);
  });
}

function mesclarRegistros(remotos, locais) {
  const mapa = new Map();
  [...locais, ...remotos].forEach((item) => {
    const registro = normalizarRegistroRelatorio(item);
    const chave = registro.protocolo || [registro.dataISO, registro.colaborador, registro.registro, registro.setorPosto, registro.quantidade].join('|');
    mapa.set(chave, registro);
  });
  return [...mapa.values()].sort((a, b) => `${b.dataISO}${b.horaEnvio}`.localeCompare(`${a.dataISO}${a.horaEnvio}`));
}
...
async function gerarRelatorio() {
  ...
  if (!inicio || !fim || inicio > fim) {
    toast('Informe um período válido para o relatório.', 'erro');
    return;
  }

  setLoadingBotao('btnGerarRelatorio', true, 'Consultando...');
  const locais = filtrarHistoricoLocal(inicio, fim, turno);
  let remotos = [];
  try {
    if (navigator.onLine) {
      const resposta = await apiGet('relatorio', { dataInicio: inicio, dataFim: fim, turno, equipe: APP.equipe || CONFIG.equipe });
      remotos = Array.isArray(resposta.registros) ? resposta.registros : [];
    }
  } catch (erro) {
    toast(`Relatório exibido com o histórico deste aparelho. ${erro.message}`, 'aviso', 5000);
  } finally {
    setLoadingBotao('btnGerarRelatorio', false);
  }

  relatorioAtual = mesclarRegistros(remotos, locais);
  renderizarRelatorio(relatorioAtual);
}

function renderizarRelatorio(registros) {
  const total = registros.reduce((soma, item) => soma + Number(item.quantidade || 0), 0);
  const colaboradores = new Set(registros.map((item) => item.colaborador).filter(Boolean)).size;
  const setores = new Set(registros.map((item) => item.setorPosto).filter(Boolean)).size;
  const dias = new Set(registros.map((item) => item.dataISO).filter(Boolean)).size;

  $('resumoRelatorio').innerHTML = `
    <div class="resumo-card"><span>Total de cargas</span><strong>${total.toLocaleString('pt-BR')}</strong></div>
    <div class="resumo-card"><span>Lançamentos</span><strong>${registros.length.toLocaleString('pt-BR')}</strong></div>
    <div class="resumo-card"><span>Colaboradores</span><strong>${colaboradores.toLocaleString('pt-BR')}</strong></div>
    <div class="resumo-card"><span>Dias com registro</span><strong>${dias.toLocaleString('pt-BR')}</strong></div>
  `;

  if (!registros.length) {
    $('tabelaRelatorio').innerHTML = '<div class="resultado-vazio">Nenhum lançamento encontrado no período.</div>';
    return;
  }

  $('tabelaRelatorio').innerHTML = `
    <table class="tabela-relatorio">
      <thead><tr><th>Data</th><th>Colaborador</th><th>Registro</th><th>Turno</th><th>Setor / posto</th><th>Protocolo</th><th>Quantidade</th></tr></thead>
      <tbody>${registros.map((item) => `
        <tr>
          <td>${escaparHTML(item.data || formatarDataBR(item.dataISO))}</td>
          <td>${escaparHTML(item.colaborador)}</td>
          <td>${escaparHTML(item.registro)}</td>
          <td>${escaparHTML(item.turno)}</td>
          <td>${escaparHTML(item.setorPosto)}</td>
...
function exportarRelatorioCSV() {
  if (!relatorioAtual.length) {
    toast('Gere o relatório antes de exportar.', 'aviso');
    return;
  }
  const linhas = [
    ['Data', 'Colaborador', 'Registro', 'Turno', 'Setor/Posto', 'Protocolo', 'Quantidade'],
    ...relatorioAtual.map((item) => [item.data, item.colaborador, item.registro, item.turno, item.setorPosto, item.protocolo, item.quantidade])
  ];
  const csv = '\ufeff' + linhas.map((linha) => linha.map((valor) => `"${String(valor ?? '').replaceAll('"', '""')}"`).join(';')).join('\r\n');
  const blob = new Blob([csv], { type: 'text/csv;charset=utf-8' });
  const link = document.createElement('a');
  link.href = URL.createObjectURL(blob);
  link.download = `relatorio-cargas-${$('relatorioDataInicio').value}-a-${$('relatorioDataFim').value}.csv`;
  link.click();
...
function imprimirRelatorio() {
  if (!relatorioAtual.length) {
    toast('Gere o relatório antes de imprimir.', 'aviso');
    return;
  }
  window.print();
}
...
async function salvarColaborador() {
  ...
  if (!pessoa) {
    if (pessoasAtivas().some((item) => item.nome === nome && item.turno === turno)) {
...
async function salvarEscalaMensal() {
  ...
  pessoa.dias = Array.from({ length: 31 }, (_, indice) => {
    const dia = indice + 1;
    return dia <= diasNoMes() ? ($(`diaAdmin_${dia}`)?.value || '') : '';
  });
  salvarCacheMes();
  analisarConflitos();
  atualizarInterfaceCompleta({ preservarAdmin: true });
  await apiPost({ acao: 'salvarEscala', senha: adminSenhaSessao, equipe: APP.equipe || CONFIG.equipe, mesAno: mesSelecionado, colaborador: pessoa });
  msgAdmin(`Escala mensal de ${pessoa.nome} salva.`);
  toast('Escala mensal salva.', 'ok');
}
...
async function substituirColaborador() {
  ...
  await apiPost({
    ...
    mesAno: mesSelecionado, origemId: origem.id, destinoId: destino.id, diaInicio
  });
  msgAdmin(`${destino.nome} assumiu a escala a partir do dia ${String(diaInicio).padStart(2, '0')}.`);
}

async function copiarMesAtual() {
  if (!exigirAdmin()) return;
  const destino = $('adminMesDestino')?.value || '';
  if (!destino || destino === mesSelecionado) {
    msgAdmin('Escolha um mês diferente para receber a cópia.', 'erro');
    return;
  }
  if (!confirm(`Copiar a escala de ${formatarMes(mesSelecionado)} para ${formatarMes(destino)}?`)) return;

  const pacote = { mesAno: destino, legenda: clonar(LEGENDA), escala: clonar(ESCALA), atualizadoEm: new Date().toISOString(), origem: 'cópia local' };
  localStorage.setItem(chaveCache(destino), JSON.stringify(pacote));
  await apiPost({ acao: 'copiarMes', senha: adminSenhaSessao, equipe: APP.equipe || CONFIG.equipe, mesOrigem: mesSelecionado, mesDestino: destino });
  await carregarMes(destino, { forcar: false, silencioso: true });
  msgAdmin(`Escala copiada para ${formatarMes(destino)}.`);
  toast('Novo mês criado.', 'ok');
}
...
async function sincronizarDadosComPlanilha() {
  ...
  await apiPost({ acao: 'sincronizarTudo', senha: adminSenhaSessao, equipe: APP.equipe || CONFIG.equipe, mesAno: mesSelecionado, postos: LEGENDA, escala: ESCALA });
...
function analisarConflitos() {
  ...
  campo.innerHTML = conflitos.length
    ...
    : '<div class="conflito-ok">Nenhuma duplicidade de posto encontrada neste mês.</div>';
}
...
async function iniciar() {
  if ($('versaoApp')) $('versaoApp').textContent = `Versão ${APP.versao || '2.0.0'}`;
  atualizarStatusConexao();
  atualizarFilaOfflineInfo();
  prepararRelatorios();

  mesSelecionado = localStorage.getItem(STORAGE.mes) || mesAtualISO();
  if (!/^\d{4}-\d{2}$/.test(mesSelecionado)) mesSelecionado = mesAtualISO();
  $('mesConsulta').value = mesSelecionado;
  $('adminMesDestino').value = mesAtualISO();
  $('cargaData').value = dataISOHoje();

  await carregarMes(mesSelecionado, { forcar: true, silencioso: true });
  restaurarRascunhoCarga();
  await mostrarHoje(false);
...
document.addEventListener('input', (evento) => {
  if (['cargaColaborador', 'cargaData', 'cargaRegistro', 'cargaQuantidade'].includes(evento.target?.id)) salvarRascunhoCarga();
});
