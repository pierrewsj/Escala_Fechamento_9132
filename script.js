...
async function mostrarEscalaSabado(dataISO, botao = null) {
  ...
  const itens = (escala.itens || []).filter((item) => item.status === 'PUBLICADA' && (turnoSelecionado === 'todos' || item.turno === turnoSelecionado));
...
function renderizarEditorEscalaSabado(escala = null) {
  ...
  const pessoas = pessoasAtivas().filter((pessoa) => pessoa.turno === turno)
    .slice().sort((a, b) => a.nome.localeCompare(b.nome, 'pt-BR'));
  ...
  lista.innerHTML = pessoas.length ? pessoas.map((pessoa, indice) => {
    ...
    return `<div class="linha-he ${selecionado ? '' : 'inativa'}" data-colaborador-id="${escaparHTML(pessoa.id)}">
      ...
      <label class="pessoa-he" for="participanteHE_${indice}"><strong>${escaparHTML(pessoa.nome)}</strong><small>${escaparHTML(pessoa.registro || 'Registro não cadastrado')} • ${escaparHTML(pessoa.turno)}</small></label>
...
async function carregarEditorEscalaSabado(forcar = true) {
  ...
  if (carregandoSabado) return;
  carregandoSabado = true;
  const turno = $('turnoEscalaSabado')?.value || CONFIG.turno;
  if ($('listaEscalaSabado')) $('listaEscalaSabado').innerHTML = '<div class="resultado-vazio">Carregando escala de sábado...</div>';
  try {
    escalaSabadoEditor = await carregarEscalaSabado(dataISO, turno, { forcar, incluirRascunho: true, silencioso: false });
    renderizarEditorEscalaSabado(escalaSabadoEditor);
  } finally { carregandoSabado = false; }
}
...
function atualizarResumoSabado() {
  ...
  if ($('resumoEscalaSabado')) $('resumoEscalaSabado').textContent = `${itens.length} colaborador(es) selecionado(s) • ${semPosto} sem posto • ${conflitos.length} possível(is) conflito(s)`;
...
function preencherPostosEscalaFixaSabado() {
  ...
  linhasEditorSabado().forEach((linha) => {
    const pessoa = colaboradorPorId(linha.dataset.colaboradorId || '');
    const posto = pessoa?.dias[dia - 1] || '';
    if (!pessoa || !posto || SITUACOES.has(posto)) return;
  ...
  atualizarResumoSabado();
  toast(`${preenchidos} posição(ões) preenchida(s) com base na escala fixa do dia ${String(dia).padStart(2, '0')}.`, 'info');
}
...
async function iniciar() {
  if ($('versaoApp')) $('versaoApp').textContent = `Versão ${APP.versao || '2.2.0'}`;
  atualizarStatusConexao(); atualizarFilaOfflineInfo(); migrarCacheEscalaFixa(); mesSelecionado = CHAVE_ESCALA_FIXA;
  if ($('cargaData')) $('cargaData').value = dataISOHoje();
  await carregarEscalaFixa({ forcar: true, silencioso: true });
  restaurarRascunhoCarga(); await mostrarHoje(); registrarServiceWorker(); processarFilaOffline(); ocultarCarregamentoInicial();
}
