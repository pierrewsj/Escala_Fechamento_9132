...
function modalAppEstaVisivel(modal) {
  if (!modal || modal.hidden) return false;
  const estilo = globalThis.getComputedStyle?.(modal);
  return !estilo || (estilo.display !== 'none' && estilo.visibility !== 'hidden');
}

function sincronizarEstadoModalBody() {
  const algumAberto = Array.from(document.querySelectorAll('.modal-app')).some(modalAppEstaVisivel);
  document.body.classList.toggle('modal-aberto', algumAberto);
  if (!algumAberto) {
    // Recuperação defensiva: versões anteriores podiam deixar a página travada
    // com overflow inline/classe mesmo depois do fechamento do modal.
...
function instalarRecuperacaoRolagemGlobal() {
  // Revalida automaticamente toda vez que um modal muda de estado.
  const observer = new MutationObserver(() => sincronizarEstadoModalBody());
  document.querySelectorAll('.modal-app').forEach((modal) => {
    observer.observe(modal, { attributes: true, attributeFilter: ['hidden', 'style', 'class'] });
  });

  // Android pode restaurar uma página do cache com a classe antiga no body.
  window.addEventListener('pageshow', () => sincronizarEstadoModalBody());
  document.addEventListener('visibilitychange', () => {
    if (!document.hidden) sincronizarEstadoModalBody();
  });

  // Se a tela não estiver com modal visível, qualquer tentativa de rolagem
  // também corrige imediatamente um bloqueio residual.
  document.addEventListener('touchstart', () => sincronizarEstadoModalBody(), { capture: true, passive: true });
}
...
function mesclarPessoaDuplicada(base, candidata) {
  ...
  return {
    ...
    referenciaVaga: referenciaVagaAberta(preferida) || referenciaVagaAberta(candidata) || referenciaVagaAberta(base) || '',
...
function normalizarDadosAtuais() {
  ...
  ESCALA = deduplicarEscalaLocal((Array.isArray(ESCALA) ? ESCALA : []).map((pessoa) => ({
    ...
    registro: ehVagaAberta(pessoa) ? '' : String(pessoa.registro || '').trim(),
    referenciaVaga: ehVagaAberta(pessoa) ? referenciaVagaAberta(pessoa) : '',
...
function abrirAba(id, botao = null) {
  ...
  requestAnimationFrame(() => sincronizarEstadoModalBody());
}
...
async function iniciar() {
  if ($('versaoApp')) $('versaoApp').textContent = `Versão ${APP.versao || '2.6.9'}`;
  // Nunca herdar bloqueio de rolagem de uma sessão/PWA anterior.
  document.body.classList.remove('modal-aberto');
  document.body.style.removeProperty('overflow');
  document.documentElement.style.removeProperty('overflow');
  instalarRecuperacaoRolagemGlobal();
