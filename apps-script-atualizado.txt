...
const CABECALHOS = Object.freeze({
  ...
  colaboradores: ['ID', 'Nome', 'Registro', 'Turno', 'Ativo', 'Atualizado Em', 'Antigo ocupante'],
  ...
  escala: ['Referência', 'ID Colaborador', 'Nome', 'Registro', 'Turno']
    ...
    .concat(['Ativo', 'Atualizado Em', 'Antigo ocupante']),
  ...
  try {
    ...
    if (acao === 'salvarFechamento') {
      ...
      switch (acao) {
        ...
        case 'assumirPosicao': resposta = assumirPosicao_(dados); break;
...
function configurarSistema() {
  ...
  return `Sistema configurado com sucesso. Registros migrados/ajustados: ${migradas || 0}.`;
}
...
function garantirEstruturaSeNecessario_() {
  ...
  const referenciaVagaAtualizada = Boolean(
    abaColaboradores && abaColaboradores.getMaxColumns() >= CABECALHOS.colaboradores.length
    && String(abaColaboradores.getRange(1, 7).getValue() || '') === 'Antigo ocupante'
    && abaEscala && abaEscala.getMaxColumns() >= CABECALHOS.escala.length
    && String(abaEscala.getRange(1, 39).getValue() || '') === 'Antigo ocupante'
  );
  if (!abasPresentes || !coberturaAtualizada || !referenciaVagaAtualizada) garantirEstrutura_();
}
...
function garantirEstrutura_() {
  ...
  garantirEscalaFixa_();
  const migradasSituacoes = migrarSituacoesLegadas_();
  const migradasVagas = migrarReferenciasVagas_();
  return Number(migradasSituacoes || 0) + Number(migradasVagas || 0);
}

function migrarReferenciasVagas_() {
  const planilha = SpreadsheetApp.getActiveSpreadsheet();
  const abaCol = planilha.getSheetByName(NOMES_ABAS.colaboradores);
  const abaEscala = planilha.getSheetByName(NOMES_ABAS.escala);
  let migradas = 0;

  if (abaCol && abaCol.getLastRow() > 1) {
    const qtd = abaCol.getLastRow() - 1;
    const valores = abaCol.getRange(2, 1, qtd, CABECALHOS.colaboradores.length).getValues();
    valores.forEach((linha, indice) => {
      const id = String(linha[0] || '').trim().toLowerCase();
      const nome = textoChavePessoa_(linha[1] || '');
      const vaga = id.indexOf('vaga-') === 0 || nome === 'A DEFINIR';
      if (!vaga) return;
      const registro = String(linha[2] || '').trim();
      const referenciaAtual = limparReferenciaVaga_(linha[6] || '');
      const referenciaLegada = registro.toUpperCase().indexOf(PREFIXO_REFERENCIA_VAGA_) === 0 ? limparReferenciaVaga_(registro) : '';
      const referencia = referenciaAtual || referenciaLegada;
      if (!referencia) return;
      if (registro.toUpperCase().indexOf(PREFIXO_REFERENCIA_VAGA_) === 0) abaCol.getRange(indice + 2, 3).clearContent();
      abaCol.getRange(indice + 2, 7).setValue(referencia);
      migradas += 1;
    });
  }

  if (abaEscala && abaEscala.getLastRow() > 1) {
    const qtd = abaEscala.getLastRow() - 1;
    const valores = abaEscala.getRange(2, 1, qtd, CABECALHOS.escala.length).getValues();
    valores.forEach((linha, indice) => {
      const id = String(linha[1] || '').trim().toLowerCase();
      const nome = textoChavePessoa_(linha[2] || '');
      const vaga = id.indexOf('vaga-') === 0 || nome === 'A DEFINIR';
      if (!vaga) return;
      const registro = String(linha[3] || '').trim();
      const referenciaAtual = limparReferenciaVaga_(linha[38] || '');
      const referenciaLegada = registro.toUpperCase().indexOf(PREFIXO_REFERENCIA_VAGA_) === 0 ? limparReferenciaVaga_(registro) : '';
      const referencia = referenciaAtual || referenciaLegada;
      if (!referencia) return;
      if (registro.toUpperCase().indexOf(PREFIXO_REFERENCIA_VAGA_) === 0) abaEscala.getRange(indice + 2, 4).clearContent();
      abaEscala.getRange(indice + 2, 39).setValue(referencia);
      migradas += 1;
    });
  }

  if (migradas) registrarHistorico_('MIGRAÇÃO REFERÊNCIA DE VAGAS', `${migradas} referência(s) movidas da coluna Registro para Antigo ocupante.`, 'Sistema');
  return migradas;
}
...
function prepararAba_(planilha, nome, cabecalhos) {
  ...
  if (nome === NOMES_ABAS.colaboradores) aba.setColumnWidth(7, 170);
  if (nome === NOMES_ABAS.escala) aba.setColumnWidth(39, 170);
...
function limparReferenciaVaga_(valor) {
  const texto = String(valor || '').trim();
  if (!texto) return '';
  return texto.toUpperCase().indexOf(PREFIXO_REFERENCIA_VAGA_) === 0
    ? texto.slice(PREFIXO_REFERENCIA_VAGA_.length).trim().toUpperCase()
    : texto.toUpperCase();
}

function referenciaVagaAberta_(item) {
  if (!ehVagaAberta_(item)) return '';
  const direta = limparReferenciaVaga_(item && item.referenciaVaga || '');
  if (direta) return direta;
  // Compatibilidade com versões antigas, que guardavam VAGA_DE|NOME em Registro.
  const registro = String(item && item.registro || '').trim();
  return registro.toUpperCase().indexOf(PREFIXO_REFERENCIA_VAGA_) === 0
    ? limparReferenciaVaga_(registro)
    : '';
}
...
function obterBootstrap_() {
  ...
  const escalasAtivas = valores
    ...
    .map(linha => ({
      ...
      referenciaVaga: limparReferenciaVaga_(linha[38] || '')
    }))
  ...
  const escala = colaboradores.map(item => {
    ...
    if (existente) {
      ...
      return {
        ...
        registro: ehVagaAberta_(item) ? '' : (item.registro || existente.registro),
        referenciaVaga: ehVagaAberta_(item) ? (referenciaVagaAberta_(item) || referenciaVagaAberta_(existente)) : '',
        turno,
        dias: existente.dias,
        ativo: true
      };
    ...
    // Colaborador ativo sem escala é permitido: pode ser novato ou estar aguardando realocação.
    return {
      id: item.id,
      nome: item.nome,
      registro: ehVagaAberta_(item) ? '' : item.registro,
      referenciaVaga: ehVagaAberta_(item) ? referenciaVagaAberta_(item) : '',
      turno,
      dias: Array(31).fill(''),
      ativo: true
    };
...
function salvarColaborador_(dados) {
  ...
  const agora = new Date();
  const linha = [pessoa.id, String(pessoa.nome).toUpperCase(), ehVagaAberta_(pessoa) ? '' : (pessoa.registro || ''), pessoa.turno, pessoa.ativo !== false, agora, ehVagaAberta_(pessoa) ? referenciaVagaAberta_(pessoa) : ''];
...
function retirarColaborador_(dados) {
  ...
  const referenciaVaga = String(pessoa.nome || '').trim().toUpperCase();
  upsertPorChave_(NOMES_ABAS.colaboradores, CABECALHOS.colaboradores.length, 1, vagaId, [vagaId, 'A DEFINIR', '', pessoa.turno, true, agora, referenciaVaga]);
...
function assumirPosicao_(dados) {
  const novoColaboradorId = String(dados.novoColaboradorId || '').trim();
  const colaboradorSubstituidoId = String(dados.colaboradorSubstituidoId || '').trim();
  const destinoOcupante = String(dados.destinoOcupante || 'MANTER').trim().toUpperCase() === 'RETIRAR' ? 'RETIRAR' : 'MANTER';
  if (!novoColaboradorId || !colaboradorSubstituidoId) throw new Error('Informe o colaborador que assumirá e o ocupante atual da posição.');
  if (novoColaboradorId === colaboradorSubstituidoId) throw new Error('Escolha dois colaboradores diferentes.');

  const planilha = SpreadsheetApp.getActiveSpreadsheet();
  const abaCol = planilha.getSheetByName(NOMES_ABAS.colaboradores);
  const abaEscala = planilha.getSheetByName(NOMES_ABAS.escala);
  const linhaNovoCol = localizarLinha_(abaCol, 1, novoColaboradorId);
  const linhaAtualCol = localizarLinha_(abaCol, 1, colaboradorSubstituidoId);
  if (!linhaNovoCol) throw new Error('Colaborador que assumirá não encontrado.');
  if (!linhaAtualCol) throw new Error('Ocupante atual não encontrado.');
  ...
  const novo = lerPessoa(linhaNovoCol);
  const atual = lerPessoa(linhaAtualCol);
  if (!novo.ativo || !atual.ativo) throw new Error('Os dois colaboradores precisam estar ativos antes da substituição.');
  if (ehVagaAberta_(novo) || ehVagaAberta_(atual)) throw new Error('Esta operação é exclusiva para colaboradores reais. Para A DEFINIR use Preencher vaga.');

  const valoresEscala = obterValoresSemCabecalho_(abaEscala, CABECALHOS.escala.length);
  const encontrarCandidatas = (pessoa) => {
    const candidatas = [];
    valoresEscala.forEach((linha, indice) => {
      if (String(linha[0] || '') !== CHAVE_ESCALA_FIXA || !valorBooleano_(linha[36])) return;
      const idEscala = String(linha[1] || '').trim();
  ...
  const candidatasNovo = encontrarCandidatas(novo);
  if (candidatasNovo.some(item => item.linha.slice(5, 36).some(valor => String(valor || '').trim()))) {
    throw new Error(`${novo.nome} já possui uma escala fixa. Para esse caso use Trocar posições ou Realocar para vaga.`);
  }
  const candidatasAtual = encontrarCandidatas(atual);
  const melhorAtual = candidatasAtual.slice().sort((x, y) => y.linha.slice(5, 36).filter(Boolean).length - x.linha.slice(5, 36).filter(Boolean).length)[0] || null;
  if (!melhorAtual) throw new Error(`A escala fixa de ${atual.nome} não foi encontrada.`);
  const diasRecebidos = melhorAtual.linha.slice(5, 36).map(valor => String(valor || '').trim());
  if (!diasRecebidos.some(Boolean)) throw new Error(`${atual.nome} não possui postos definidos para transferir.`);

  const turnoAnteriorNovo = novo.turno;
  const turnoDestino = atual.turno;
  const mudaTurno = !mesmoTurno_(turnoAnteriorNovo, turnoDestino);
  const agora = new Date();

  // Quem assume passa a ocupar a posição e o turno vinculados à escala recebida.
  abaCol.getRange(linhaNovoCol, 4).setValue(turnoDestino);
  abaCol.getRange(linhaNovoCol, 5).setValue(true);
  abaCol.getRange(linhaNovoCol, 6).setValue(agora);
  novo.turno = turnoDestino;

  // Situações continuam ligadas à pessoa. Ajusta apenas o turno do novo colaborador quando necessário.
  if (mudaTurno) {
    ...
    if (abaSituacoes && abaSituacoes.getLastRow() > 1) {
      ...
      valoresSituacoes.forEach((linha, indice) => {
        ...
        if (String(linha[1] || '').trim() === novo.id) {
          abaSituacoes.getRange(indice + 2, 4).setValue(turnoDestino);
  ...
  // A posição antiga deixa de pertencer ao ocupante atual.
  candidatasAtual.forEach(item => {
    abaEscala.getRange(item.indice + 2, 37).setValue(false);
    abaEscala.getRange(item.indice + 2, 38).setValue(agora);
  });

  if (destinoOcupante === 'RETIRAR') {
    // Sai efetivamente da equipe.
    abaCol.getRange(linhaAtualCol, 5).setValue(false);
    abaCol.getRange(linhaAtualCol, 6).setValue(agora);
  } else {
    // Continua ativo, mas sem uma posição definida. Isso é um estado válido, não uma vaga A DEFINIR.
    abaCol.getRange(linhaAtualCol, 5).setValue(true);
    abaCol.getRange(linhaAtualCol, 6).setValue(agora);
    salvarLinhaEscala_(CHAVE_ESCALA_FIXA, { ...atual, dias: Array(31).fill(''), ativo: true });
  }

  // Grava a posição no novo ocupante.
  salvarLinhaEscala_(CHAVE_ESCALA_FIXA, { ...novo, dias: diasRecebidos, ativo: true });

  const destinoTexto = destinoOcupante === 'RETIRAR'
    ? `${atual.nome} foi retirado da equipe.`
    : `${atual.nome} permaneceu ativo, sem escala fixa e aguardando nova posição.`;
  registrarHistorico_('ASSUMIR POSIÇÃO', `${novo.nome}${mudaTurno ? ` (${turnoAnteriorNovo}→${turnoDestino})` : ` • ${turnoDestino}`} assumiu a posição de ${atual.nome}; ${destinoTexto}`, 'Administrador');
  return {
    sucesso: true,
    ...
    destinoOcupante,
    turno: turnoDestino,
    mensagem: `${novo.nome} assumiu a posição de ${atual.nome}. ${destinoTexto}`
  };
}
...
function realocarColaborador_(dados) {
  ...
  const referenciaNovaVaga = String(pessoa.nome || '').trim().toUpperCase();
  upsertPorChave_(NOMES_ABAS.colaboradores, CABECALHOS.colaboradores.length, 1, novaVagaId, [novaVagaId, 'A DEFINIR', '', pessoa.turno, true, agora, referenciaNovaVaga]);
  salvarLinhaEscala_(CHAVE_ESCALA_FIXA, { id: novaVagaId, nome: 'A DEFINIR', registro: '', referenciaVaga: referenciaNovaVaga, turno: pessoa.turno, ativo: true, dias: diasAtuais });
...
function salvarLinhaEscala_(mesAno, pessoa) {
  ...
  const registro = vaga ? '' : String(pessoa.registro || '').trim();
  const referenciaVaga = vaga ? referenciaVagaAberta_(pessoa) : '';
  const nome = String(pessoa.nome || '').trim().toUpperCase();
  const linha = [mesAno, pessoa.id, nome, registro, turno]
    .concat(dias)
    .concat([pessoa.ativo !== false, new Date(), referenciaVaga]);
  ...
  valores.forEach((existente, indice) => {
    ...
    const pessoaEhVaga = vaga;
    const mesmoRegistro = !pessoaEhVaga && registro && registroExistente === registro && mesmoTurno_(turnoExistente, turno);
    const mesmoNomeSemRegistro = !pessoaEhVaga && !registro && !registroExistente && textoChavePessoa_(nomeExistente) === textoChavePessoa_(nome) && mesmoTurno_(turnoExistente, turno);
...
function lerColaboradoresAtivos_() {
  ...
  return obterValoresSemCabecalho_(aba, CABECALHOS.colaboradores.length)
    .map(linha => ({
      ...
      referenciaVaga: limparReferenciaVaga_(linha[6] || '')
    }))
