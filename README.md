## Versão 2.6.9 — correção das colunas das vagas

- A referência **vaga de PIERRE / vaga de RENATO** não é mais gravada na coluna Registro.
- As abas **Colaboradores** e **Escala** recebem uma coluna própria chamada **Antigo ocupante** no final da tabela.
- Ao executar `configurarSistema`, referências antigas no formato `VAGA_DE|NOME` são migradas automaticamente para a nova coluna e a coluna Registro é limpa nas vagas A DEFINIR.
- Registros funcionais reais continuam exclusivamente na coluna Registro.
- Linhas A DEFINIR com `Ativo = FALSE` são registros antigos/inativos e não entram no aplicativo; não são apagadas automaticamente para preservar histórico.
- Esta versão exige atualização do Apps Script para 2.6.9 e execução única de `configurarSistema`.

## Versão 2.6.8 — correção de rolagem no celular

- Corrigido bloqueio em que a página podia permanecer com `modal-aberto` depois de fechar um modal.
- Ao trocar entre Hoje, Consulta, Cargas, Sábado HE, Legenda ou Admin, o estado de rolagem é revalidado.
- Ao restaurar o PWA do cache do Android, qualquer bloqueio residual é limpo automaticamente.
- Um observador acompanha abertura/fechamento dos modais e mantém a trava somente enquanto existe um modal realmente visível.
- Fora dos modais, a rolagem vertical por toque fica explicitamente habilitada.
- Não exige alteração do Apps Script; permanece compatível com o Apps Script 2.6.6.

## Versão 2.6.7 — seleção inteligente de colaborador sem escala

- Em **Trocar posições**, colaboradores sem escala fixa (como um novato) deixam de ficar bloqueados.
- Ao tocar em alguém sem escala, o aplicativo explica que não existe uma posição para trocar e oferece abrir diretamente **Assumir posição** com essa pessoa já selecionada.
- O fluxo de duas pessoas com escala fixa continua exatamente igual.
- Esta versão é apenas de interface/lógica do aplicativo; o Apps Script 2.6.6 continua válido.
